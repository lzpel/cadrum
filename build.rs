...
fn link_occt_libraries(occt_include: &Path, occt_lib_dir: &Path, target: &str) {
	...
	// The C ABI wrapper (src/ffi.cpp against the bridge()-generated ffi.h) is compiled with plain cc(#235).
	let path_ffi_h = ffi::bridge(Path::new("src/ffi.rs"));
	let mut build = cc::Build::new();
	build.cpp(true).file("src/ffi.cpp").include(path_ffi_h.parent().unwrap()).include(occt_include).std("c++17").define("_USE_MATH_DEFINES", None);

	apply_compiler_flags(|s| {
		build.flag(s);
	});

	// Mirror every enabled cargo feature as a FEATURE_<NAME> define
...
// ---------------------------------------------------------------------------
// ffi: generate OUT_DIR/ffi.h from src/ffi.rs. The dependency on syn lives
// here only.
// ---------------------------------------------------------------------------
mod ffi {
	use std::env;
	...
	/// Generate the C header for the FFI from `src` (src/ffi.rs) into `OUT_DIR/ffi.h` and return the generated header's path (callers add its `parent()` as an include dir).
	///
	/// src/ffi.rs is the single source of truth for the ABI: the `mod raw` extern
	/// block becomes prototypes the C++ side must implement, and the top-level
	/// `#[no_mangle] extern "C"` fns become prototypes for the Rust-implemented
	/// callbacks C++ calls. Since src/ffi.cpp includes the generated header, any
	/// signature drift between ffi.rs and the handwritten shims is a conflicting
	/// extern "C" declaration — a C++ compile error, like cxx-build used to give.
	pub fn bridge(src: &Path) -> PathBuf {
		let source = std::fs::read_to_string(src).unwrap_or_else(|e| panic!("bridge: read {}: {e}", src.display()));
		let file = syn::parse_file(&source).unwrap_or_else(|e| panic!("bridge: parse {}: {e}", src.display()));

		let mut rust_impl = String::new(); // #[no_mangle] fns (Rust implements, C++ calls)
		let mut cpp_impl = String::new(); // mod raw externs (C++ implements, Rust calls)

		for item in &file.items {
			match item {
				syn::Item::Fn(f) if f.attrs.iter().any(|a| a.path().is_ident("no_mangle")) => {
					let abi_c = f.sig.abi.as_ref().and_then(|a| a.name.as_ref()).map(|n| n.value() == "C").unwrap_or(false);
					assert!(abi_c, "bridge: #[no_mangle] fn {} must be extern \"C\"", f.sig.ident);
					emit_prototype(&mut rust_impl, &f.attrs, &f.sig);
				}
				syn::Item::Mod(m) if m.ident == "raw" => {
					let items = &m.content.as_ref().expect("bridge: mod raw must be inline").1;
					for item in items {
						if let syn::Item::ForeignMod(fm) = item {
							for fi in &fm.items {
								match fi {
									syn::ForeignItem::Fn(f) => emit_prototype(&mut cpp_impl, &f.attrs, &f.sig),
									_ => panic!("bridge: mod raw must contain only fn declarations"),
								}
							}
						}
					}
		...
		let header = format!(
			"// Generated by build.rs bridge() from src/ffi.rs — do not edit.\n\
		 #pragma once\n\
		 \n\
		 #include <stddef.h>\n\
		 #include <stdint.h>\n\
		 \n\
		 // Pointer-only usage: forward declarations suffice (ffi.cpp includes\n\
		 // the real OCCT headers); std::vector cannot be forward-declared.\n\
		 #include <vector>\n\
		 class TopoDS_Shape;\n\
		 class TopoDS_Face;\n\
		 class TopoDS_Edge;\n\
		 ...
		 extern \"C\" {{\n\
		 \n\
		 // ==== Implemented in Rust (src/ffi.rs) — callbacks C++ calls ====\n\
		 \n{rust_impl}\
		 // ==== Implemented in C++ (src/ffi.cpp) ====\n\
		 \n{cpp_impl}\
		 }} // extern \"C\"\n"
		);

		let path = PathBuf::from(env::var("OUT_DIR").unwrap()).join("ffi.h");
		std::fs::write(&path, header).expect("bridge: write ffi.h");
		path
	}

	/// Append one C prototype to `out`, wrapped in the `#ifdef` guard derived
	/// from a `#[cfg(feature = "...")]` attribute. Doc comments are intentionally
	/// NOT emitted: the generated header is read only by the compiler — humans
	/// read the docs where they live, on the declarations in src/ffi.rs.
	fn emit_prototype(out: &mut String, attrs: &[syn::Attribute], sig: &syn::Signature) {
		// `#[cfg(feature = "x")]` → `#ifdef FEATURE_X` / `not(...)` → `#ifndef`,
		// matching the FEATURE_<NAME> defines main() derives from CARGO_FEATURE_*.
		let mut guard = None;
		for attr in attrs {
			if attr.path().is_ident("cfg") {
				let syn::Meta::List(ml) = &attr.meta else { panic!("bridge: unsupported cfg on {}", sig.ident) };
				// Token spacing varies; compare with whitespace stripped.
				let cfg: String = ml.tokens.to_string().chars().filter(|c| !c.is_whitespace()).collect();
				let (directive, feature) = if let Some(inner) = cfg.strip_prefix("not(").and_then(|s| s.strip_suffix(')')) { ("#ifndef", inner.to_string()) } else { ("#ifdef", cfg) };
				let name = feature.strip_prefix("feature=\"").and_then(|s| s.strip_suffix('"')).unwrap_or_else(|| panic!("bridge: unsupported cfg `{feature}` on {}", sig.ident));
				guard = Some(format!("{directive} FEATURE_{}\n", name.to_uppercase().replace('-', "_")));
			}
		}

		let ret = match &sig.output {
			syn::ReturnType::Default => "void".to_string(),
			syn::ReturnType::Type(_, ty) => c_type(ty, &sig.ident),
		};
		let args = sig
			.inputs
			.iter()
			.map(|arg| {
				let syn::FnArg::Typed(t) = arg else { panic!("bridge: self arg in {}", sig.ident) };
				let syn::Pat::Ident(name) = t.pat.as_ref() else { panic!("bridge: unsupported arg pattern in {}", sig.ident) };
				format!("{} {}", c_type(&t.ty, &sig.ident), name.ident)
			})
			.collect::<Vec<_>>();
		...
		out.push_str(&format!("{ret} {}({args});\n", sig.ident));
		if guard.is_some() {
			out.push_str("#endif\n");
		}
	}

	/// Map a Rust FFI type to its C spelling. Panics on anything outside the
	/// deliberately small vocabulary so an ABI-unsafe type fails the build.
	fn c_type(ty: &syn::Type, ctx: &syn::Ident) -> String {
		match ty {
			syn::Type::Ptr(p) => {
				let inner = c_type(&p.elem, ctx);
			...
			syn::Type::Path(p) => {
				let name = &p.path.segments.last().expect("empty type path").ident;
				match name.to_string().as_str() {
					...
					"bool" => "bool",
					"c_void" => "void",
					// Opaque types declared in ffi.rs. The OCCT handles keep their
					// spelling; the vector handles are Rust-side names for the real
					// std::vector types, spelled out (no typedef in the header).
					n @ ("TopoDS_Shape" | "TopoDS_Face" | "TopoDS_Edge") => n,
					"ShapeVec" => "std::vector<TopoDS_Shape>",
					"FaceVec" => "std::vector<TopoDS_Face>",
					"EdgeVec" => "std::vector<TopoDS_Edge>",
					other => panic!("bridge: unmapped type `{other}` in {ctx}"),
				}
				.to_string()
			}
			_ => panic!("bridge: unsupported type shape in {ctx} (only paths and pointers)"),
		}
	}
}
