...
mod source {
	...
	pub fn occt_from_source(effective_root: &Path) -> Option<Vec<PathBuf>> {
		...
		fn prune_except(dir: &Path, whitelist: &[PathBuf], is_white: impl Fn(&Path)->bool) -> std::io::Result<()> {
			for p in std::fs::read_dir(dir)?.filter_map(Result::ok).map(|v| v.path()) {
				// whitelist のどれかがこの path の下にある → 中へ降りて選別を続ける
				if whitelist.iter().any(|w| w.starts_with(&p)) || is_white(&p){
					match p.is_dir() {
						true=>prune_except(&p, whitelist, &is_white)?,
						false=>continue
					}
				}
				// 削除
