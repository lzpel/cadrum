...
use crate::{bounding, distance_nabla_laplacian, segment::{EdgeLoop, Segment}};
use glam::Vec2;
...
pub fn regions(sdf: impl Fn(Vec2) -> f32) -> Vec<EdgeLoop> {
	...
	let stride = RES + 1;
	let samples: Vec<f32> = (0..stride * stride)
		.map(|i| {
			let [r,c] = [i / stride, i % stride];
			let p = min + Vec2::new(c as f32 / RES as f32 * size.x, r as f32 / RES as f32 * size.y);
			sdf(p)
		})
		.collect();
	...
	let loops_proj: Vec<Vec<Vec2>> = loops_raw
		.into_iter()
	...
	let mut sorted: Vec<(f32, Vec<Vec2>)> = loops_proj
		.into_iter()
...
fn edge_crossing_pos(edge_id: u32, samples: &[f32], min: Vec2, size: Vec2) -> Vec2 {
	let stride = (RES + 1) as u32;
	...
	let cell_dx = size.x / RES as f32;
	let cell_dy = size.y / RES as f32;
	let p0 = min + Vec2::new(c as f32 * cell_dx, r as f32 * cell_dy);
	if is_vertical {
		let d1 = samples[(r + 1) * (RES + 1) + c];
		let t = d0 / (d0 - d1);
		p0 + Vec2::new(0.0, t * cell_dy)
	} else {
		let d1 = samples[r * (RES + 1) + c + 1];
		let t = d0 / (d0 - d1);
		p0 + Vec2::new(t * cell_dx, 0.0)
	}
}
	...
	for r in 0..RES {
		for c in 0..RES {
			...
			let center_inside = if code == 5 || code == 10 {
				let cx = min.x + (c as f32 + 0.5) * (size.x / RES as f32);
				let cy = min.y + (r as f32 + 0.5) * (size.y / RES as f32);
				sdf(Vec2::new(cx, cy)) < 0.0
			} else { false };
...
fn project_loop<F: Fn(Vec2) -> f32>(pts: Vec<Vec2>, sdf: &F) -> Vec<Vec2> {
	pts.into_iter()
		.map(|p| {
			let mut q = p;
...
fn run_points(i: usize, pts: &[Vec2], start_idx: &[usize], run_len: &[usize]) -> Vec<Vec2> {
	let n = pts.len();
...
fn merge_residual(
	...
	let n = pts.len();
	let j = next_arr[i];
	if i == j { return f32::INFINITY; }
	let last_of_i = (start_idx[i] + run_len[i] - 1) % n;
	if barrier[last_of_i] { return f32::INFINITY; }
...
fn fit_segments(
	...
	// コーナー検出用の勾配を必要なときに計算 (キャッシュは持たない)。
	...
	let mut merge_res: Vec<f32> = vec![f32::INFINITY; n];
	...
	if let Some(s) = first {
		let mut i = s;
		loop {
			// コーナー付近で隣接 barrier に挟まれた 2 点以下の run は corner artifact として捨てる。
			if run_len[i] >= 3 {
...
fn fit_line(pts: &[Vec2]) -> (Vec2, Vec2, f32) {
	let n = pts.len() as f32;
	let centroid = pts.iter().copied().fold(Vec2::ZERO, std::ops::Add::add) / n;
	let mut sxx = 0.0_f32; let mut sxy = 0.0_f32; let mut syy = 0.0_f32;
	for p in pts {
		let q = *p - centroid;
	...
	let dir = if sxy.abs() > 1e-12_f32 * (trace.abs() + 1.0) {
		Vec2::new(sxy, lambda_max - sxx).normalize_or_zero()
	} else if sxx >= syy { Vec2::X } else { Vec2::Y };
	let normal = Vec2::new(-dir.y, dir.x);
	let max_res = pts.iter().map(|p| ((*p - centroid).dot(normal)).abs()).fold(0.0_f32, f32::max);
	(centroid, dir, max_res)
}

/// 円フィット (Kasa 線形最小二乗、中心化版)。3点未満や共線で None。
fn fit_circle(pts: &[Vec2]) -> Option<(Vec2, f32, f32)> {
	if pts.len() < 3 { return None; }
	let n = pts.len() as f32;
	let mean = pts.iter().copied().fold(Vec2::ZERO, std::ops::Add::add) / n;
	let mut sxx = 0.0_f32; let mut sxy = 0.0_f32; let mut syy = 0.0_f32;
	let mut sxz = 0.0_f32; let mut syz = 0.0_f32; let mut sz = 0.0_f32;
	for p in pts {
	...
	let scale = sxx + syy;
	if scale <= 0.0 || det.abs() < 1e-12_f32 * scale * scale { return None; }
	...
	let radius = r2.sqrt();
	let center = Vec2::new(mean.x + 0.5 * a, mean.y + 0.5 * b);
	let max_res = pts.iter().map(|p| ((*p - center).length() - radius).abs()).fold(0.0_f32, f32::max);
...
mod tests {
	...
	fn circle() {
		...
		match l[0] {
			Segment::Circle { center, radius } => {
				assert!(center.length() < 0.0000001, "center={center}");
				assert!((radius - 1.0).abs() < 0.0000001, "radius={radius}");
			}
	...
	#[test]
	fn rectangle() {
		let [a, b] = [Vec2::new(-1.0, -1.0), Vec2::new(1.0, 1.0)];
		...
		for seg in l {
			...
			assert!([a,b].map(|v| seg.distance(v)).into_iter().any(|d| d < 0.0000001));
		}
	}

	#[test]
	fn pentagon() {
		let pent: [Vec2;5]=std::array::from_fn(|i| {
		...
		for seg in l {
			assert!(matches!(seg, Segment::Line { .. }), "expected Line");
			assert!(pent.into_iter().filter(|v| seg.distance(*v) < 0.00001).count()==2);
		}
	}
}
