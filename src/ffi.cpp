// Generated by build.rs `bridge()` from src/ffi.rs into OUT_DIR (-I).
#include "ffi.h"
...
void cadrum_shape_vec_delete(std::vector<TopoDS_Shape>* v) { delete v; }
void cadrum_face_vec_delete(std::vector<TopoDS_Face>* v) { delete v; }
void cadrum_edge_vec_delete(std::vector<TopoDS_Edge>* v) { delete v; }
...
std::vector<TopoDS_Shape>* cadrum_decompose_into_solids(const TopoDS_Shape* shape) {
...
std::vector<TopoDS_Edge>* cadrum_shape_edges(const TopoDS_Shape* shape) {
    return cadrum::shape_edges(*shape).release();
}

std::vector<TopoDS_Face>* cadrum_shape_faces(const TopoDS_Shape* shape) {
    return cadrum::shape_faces(*shape).release();
}

std::vector<TopoDS_Edge>* cadrum_face_edges(const TopoDS_Face* face) {
...
std::vector<TopoDS_Edge>* cadrum_make_polygon_edges(const double* coords, size_t coords_len) {
...
TopoDS_Shape* cadrum_make_loft(const std::vector<TopoDS_Edge>* all_edges, bool ruled) {
    return cadrum::make_loft(*all_edges, ruled).release();
}

TopoDS_Shape* cadrum_make_sewn_solid(const std::vector<TopoDS_Face>* faces, double tolerance) {
...
std::vector<TopoDS_Edge>* cadrum_edge_vec_new(void) {
    return cadrum::edge_vec_new().release();
}

void cadrum_edge_vec_push(std::vector<TopoDS_Edge>* v, const TopoDS_Edge* e) {
    cadrum::edge_vec_push(*v, *e);
}

void cadrum_edge_vec_push_null(std::vector<TopoDS_Edge>* v) {
    cadrum::edge_vec_push_null(*v);
}

size_t cadrum_edge_vec_len(const std::vector<TopoDS_Edge>* v) {
    return v->size();
}

const TopoDS_Edge* cadrum_edge_vec_get(const std::vector<TopoDS_Edge>* v, size_t i) {
    return &(*v)[i];
}

std::vector<TopoDS_Face>* cadrum_face_vec_new(void) {
    return cadrum::face_vec_new().release();
}

void cadrum_face_vec_push(std::vector<TopoDS_Face>* v, const TopoDS_Face* f) {
    cadrum::face_vec_push(*v, *f);
}

size_t cadrum_face_vec_len(const std::vector<TopoDS_Face>* v) {
    return v->size();
}

const TopoDS_Face* cadrum_face_vec_get(const std::vector<TopoDS_Face>* v, size_t i) {
    return &(*v)[i];
}

std::vector<TopoDS_Shape>* cadrum_shape_vec_new(void) {
    return cadrum::shape_vec_new().release();
}

void cadrum_shape_vec_push(std::vector<TopoDS_Shape>* v, const TopoDS_Shape* s) {
    cadrum::shape_vec_push(*v, *s);
}

size_t cadrum_shape_vec_len(const std::vector<TopoDS_Shape>* v) {
    return v->size();
}

const TopoDS_Shape* cadrum_shape_vec_get(const std::vector<TopoDS_Shape>* v, size_t i) {
    return &(*v)[i];
}
