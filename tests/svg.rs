...
fn svg_string(shape: &[Solid], direction: DVec3, tol: f64) -> String {
	...
	cadrum::Solid::mesh(shape, cadrum::Tessellation { deflection_linear: tol, relative_linear: false, ..Default::default() }).and_then(|m| m.scene(direction, up, true, false).write_svg(&mut buf)).unwrap();
